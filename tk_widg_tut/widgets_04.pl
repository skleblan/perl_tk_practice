...
#Jobs
my $frm_job = $mw -> Frame();
my $chk = $frm_job -> Checkbutton(-text=>"Occupied", -variable=>\$occupied);
$chk -> deselect();
my $lst = $frm_job -> Listbox(-selectmode=>'single');

#adding jobs
$lst -> insert('end', "Student", "Teacher", "Clerk", "Business Man",
    "Military Personel", "Computer Expert", "Other");
...
my $rdb_f = $frm_gender -> Radiobutton(-text=>"Femail",
    -value=>"Femaile", -variable=>\$gender);
...
sub push_button
{
  ...
  my $job = "";
  #see whether he/she is employed
  if($occupied == 1)
  {
    my $job_id = $lst -> curselection(); #get the index of selected jobs
    if($job_id eq "")
    {
      $job = "a non worker";
    }
